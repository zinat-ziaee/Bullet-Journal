...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    $('.futurelog-event-create-modal').click(function(){
      ...
      getDataFromTheEditor(null);//clear ckeditor
    });
  ...
  //Convert to ck editor and set data for notes
  function getDataFromTheEditor(val) {
    if (description) { 
      description.destroy();
    }
  ...
  let task_description;
    ...
    //Convert to ck editor and set data for tasks
    function setDataFromTheEditorTask(elementId,val) {
    if (task_description) { 
  ...
  $(document).on('click', '.futurelogInfoEditModal', function(e){
    ...
    if($(this).data('note-info')){
      ...
      $('#note #description').val(getDataFromTheEditor(modalData[2]));
      activaTab('#note');
      return false;
    }
    if($(this).data('task-info')){
      ...
      $('#task #description').val(setDataFromTheEditorTask('description2',modalData[2]));
      activaTab('#task');
      return false;
    }
  });
...
$(document).ready(function() {
  ...
  $(document).on('click', '#saveNode', function(e){
    ...
    async function noteDataTableUpdate(data){
      getDataFromTheEditor(data.note.description)
      const note = [
        data.note.title,
        description.getData(),
      ...
      if(noteDataTable.row('#noteItem'+data.note.id).id()){
        ...
        noteDataTable.draw();
        console.log('editeId'+noteDataTable.row('#noteItem'+data.note.id).id());
      }
      $('.default-form-class').trigger("reset");
      $('#infoModal').modal('hide'); 
    }   
